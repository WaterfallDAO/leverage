...
contract CToken is CTokenInterface, Exponential, TokenErrorReporter {
    ...
    function borrowInternal(uint borrowAmount) internal nonReentrant returns (uint) {
        if (riskControl != address(0)) {
            uint risk = RiskController(riskControl).checkBorrowRisk(address(this), msg.sender);
            require(risk == 0, 'risk control');
        }
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
    ...
    function borrowFresh(address payable borrower, uint borrowAmount) internal returns (uint) {
         ...
         *  doTransferOut reverts if anything goes wrong, since we can't be sure if side effects occurred.
         */
        uint _feeAmount = div_(mul_(borrowAmount, borrowFee), expScale);
        totalReserves = add_(totalReserves, _feeAmount);
        uint _amount = sub_(borrowAmount, _feeAmount);

        doTransferOut(borrower, _amount);
    ...
    function _setRiskControl(address _address) external returns (uint){
        require(msg.sender == admin, "!admin");
        riskControl = _address;
        return uint(Error.NO_ERROR);
    }
    ...
    function _setRedeemFeeFactorMantissa(uint _redeemFee) external returns (uint){
        require(msg.sender == admin, "!admin");
    ...
    function _setBorrowFeeFactorMantissa(uint _borrowFee) external returns (uint){
        require(msg.sender == admin, "!admin");
        require(_borrowFee > 0 && _borrowFee < 1e16, "beyond the scope");
    ...
    function _setReserveFactor(uint newReserveFactorMantissa) external nonReentrant returns (uint) {
        ...
        if (error != uint(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors,
            // but on top of that we want to log the fact that an attempted reserve factor change failed.
    ...
    function _setInterestRateModel(InterestRateModel newInterestRateModel) public returns (uint) {
        ...
        if (error != uint(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors,
            // but on top of that we want to log the fact that an attempted change of interest rate model failed
