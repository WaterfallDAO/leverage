    ...
    describe('liquidity', () => {
        ...
        it("allows a borrow up to collateralFactor, but not more", async () => {
            ...
            ({
                1: liquidity,
                2: shortfall
            } = await call(cToken.comptroller, 'getHypotheticalAccountLiquidity', [user, cToken._address, 0, amount]));
            ...
            ({
                1: liquidity,
                2: shortfall
            } = await call(cToken.comptroller, 'getHypotheticalAccountLiquidity', [user, cToken._address, amount, 0]));
            expect(liquidity).toEqualNumber(0);
            expect(shortfall).toEqualNumber(0);
        }, 20000);
        ...
        it("allows entering 3 markets, supplying to 2 and borrowing up to collateralFactor in the 3rd", async () => {
            ...
            ({
                1: liquidity,
                2: shortfall
            } = await call(cToken3.comptroller, 'getHypotheticalAccountLiquidity', [user, cToken3._address, Math.floor(c2), 0]));
            expect(liquidity).toEqualNumber(collateral);
            expect(shortfall).toEqualNumber(0);
            ...
            ({
                1: liquidity,
                2: shortfall
            } = await call(cToken3.comptroller, 'getHypotheticalAccountLiquidity', [user, cToken3._address, 0, Math.floor(c2)]));
            ...
            ({
                1: liquidity,
                2: shortfall
            } = await call(cToken3.comptroller, 'getHypotheticalAccountLiquidity', [user, cToken3._address, 0, collateral + c1]));
            expect(liquidity).toEqualNumber(0);
            ...
            ({
                1: liquidity,
                2: shortfall
            } = await call(cToken1.comptroller, 'getHypotheticalAccountLiquidity', [user, cToken1._address, amount1, 0]));
    ...
    describe("getHypotheticalAccountLiquidity", () => {
        it("returns 0 if not 'in' any markets", async () => {
            ...
            const {
                ...
                1: liquidity,
                2: shortfall
            } = await call(cToken.comptroller, 'getHypotheticalAccountLiquidity', [accounts[0], cToken._address, 0, 0]);
            expect(error).toEqualNumber(0);
            expect(liquidity).toEqualNumber(0);
            expect(shortfall).toEqualNumber(0);
        });
        ...
        it("returns collateral factor times dollar amount of tokens minted in a single market", async () => {
            ...
            const {
                ...
                1: liquidity,
                2: shortfall
            } = await call(cToken.comptroller, 'getHypotheticalAccountLiquidity', [from, cToken._address, 0, 0]);
            expect(error).toEqualNumber(0);
