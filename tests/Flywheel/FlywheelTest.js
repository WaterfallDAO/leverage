...
describe('Flywheel', () => {
    ...
    describe('updateCompBorrowIndex()', () => {
        it('should calculate comp borrower index correctly', async () => {
            ...
            // await send(comptroller, 'compBorrowSpeed', [mkt._address, etherExp(0.5)]);
    ...
    describe('updateCompSupplyIndex()', () => {
        it('should calculate comp supplier index correctly', async () => {
            ...
            */
            ...
            expect(index).toEqualNumber(3.35e36);
            expect(block).toEqualNumber(100);
        });
    ...
    describe('claimComp', () => {
        it('should accrue comp and then transfer comp accrued', async () => {
            ...
            await send(comptroller.comp, 'transfer', [comptroller._address, compRemaining], {from: root});
            await pretendBorrow(cLOW, a1, 1, 1, 100);
            let result = await send(comptroller, 'refreshCompSpeeds');
            // expect(result).toRevert('12');
            ...
            // let unclaimComp = await call(comptroller, 'unclaimed', [a2]);
            ...
            expect(speed).toEqualNumber(compRate.multipliedBy(0.47));
            expect(a2AccruedPre).toEqualNumber(0);
            expect(a2AccruedPost).toEqualNumber(0);
            expect(compBalancePre).toEqualNumber(0);
            // expect(unclaimComp).toEqualNumber(0);
            // expect(compBalancePost).toEqualNumber(compRate.multipliedBy(0.6).multipliedBy(deltaBlocks)); // index is 8333...
        });
    ...
    describe('claimComp batch', () => {
        ...
        it('should claim the expected amount when holders and ctokens arg is duplicated', async () => {
            ...
            for (let acct of claimAccts) {
                expect(await call(comptroller, 'compSupplierIndex', [cLOW._address, acct])).toEqualNumber(etherDouble(1.0276125));
                expect(await compBalance(comptroller, acct)).toEqualNumber(etherExp(0.276125));
            }
        });
        ...
        it('claims comp for multiple suppliers only', async () => {
            ...
            await fastForward(comptroller, deltaBlocks);
            await send(comptroller, 'refreshCompSpeeds', []);
            // let unclaimComp = await call(comptroller, 'unclaimed', [claimAccts[0]]);
            const tx = await send(comptroller, 'claimComp', [claimAccts, [cLOW._address], false, true]);

            // comp distributed => 10e18
            for (let acct of claimAccts) {
                expect(await call(comptroller, 'compSupplierIndex', [cLOW._address, acct])).toEqualNumber(etherDouble(1.0276125));
                expect(await compBalance(comptroller, acct)).toEqualNumber(etherExp(.276125));

            }
            // expect(unclaimComp).toEqualNumber(etherExp(.75));
        });

        it('claims comp for multiple borrowers only, primes uninitiated', async () => {
            const compRemaining = compRate.multipliedBy(100), deltaBlocks = 10, mintAmount = etherExp(10),
                borrowAmt = etherExp(1), borrowIdx = etherExp(1)
            await send(comptroller.comp, 'transfer', [comptroller._address, compRemaining], {from: root});
            let [_, __, ...claimAccts] = saddle.accounts;
            ...
            for (let acct of claimAccts) {
                expect(await call(comptroller, 'compBorrowerIndex', [cLOW._address, acct])).toEqualNumber(etherDouble(1.311375));
    ...
    describe('refreshCompSpeeds', () => {
        ...
        it('should get correct speeds for 2 assets', async () => {
            ...
            expect(speed1).toEqualNumber(compRate.dividedBy(4).multipliedBy(0.47));
            expect(speed2).toEqualNumber(0);
            expect(speed3).toEqualNumber(compRate.dividedBy(4).multipliedBy(3).multipliedBy(0.47));
        });
    ...
    describe('_setCompRate', () => {
        ...
        it('should not change comp rate unless called by admin', async () => {
            ...
            ).rejects.toRevert('revert only admin and reservoir can change comp rate');
        });
    });
});
