...
describe('CToken', function () {
    ...
    describe('balanceOfUnderlying', () => {
        ...
        it("has an underlying balance", async () => {
            const token = await makeToken({decimals: 6});
            const cToken = await makeCToken({underlying: token, decimals: 6, supportMarket: true, exchangeRate: 1});
            await send(cToken, 'harnessSetBalance', [root, 100]);
            expect(await call(cToken, 'balanceOfUnderlying', [root])).toEqualNumber(100);
        });
        it("has an underlying balance", async () => {
            const token = await makeToken({decimals: 6});
            const cToken = await makeCToken({underlying: token, decimals: 6, supportMarket: true, exchangeRate: 100});
            await send(cToken, 'harnessSetBalance', [root, 10000]);
            let balanceRoot= await call(cToken, 'balanceOf', [root]);
            expect(balanceRoot).toEqual("10000")
            expect(await call(cToken, 'balanceOfUnderlying', [root])).toEqualNumber(1000000);
        });

        it("has an underlying balance", async () => {

            const cToken = await makeCToken({kind: "cether", decimals: 6, supportMarket: true, exchangeRate: 100});
            await send(cToken, 'harnessSetBalance', [root, 10000]);
            expect(await call(cToken, 'balanceOfUnderlying', [root])).toEqualNumber(1000000);
        });
        it("has an underlying balance", async () => {

            const cToken = await makeCToken({kind: "cether", decimals: 6, supportMarket: true, exchangeRate: 1});
            await send(cToken, 'harnessSetBalance', [root, 10000]);
            expect(await call(cToken, 'balanceOfUnderlying', [root])).toEqualNumber(10000);
        });
    });
