...
async function makeComptroller(opts = {}) {
    ...
    if (kind == 'unitroller-g3') {
        ...
        // mergeInterface(unitroller, comptroller);
        await send(comptroller, '_setCompRate', [compRate]);
        await send(comptroller, '_setLiquidationIncentive', [liquidationIncentive]);
        await send(comptroller, '_setCloseFactor', [closeFactor]);
        await send(comptroller, '_setMaxAssets', [maxAssets]);
        await send(comptroller, '_setPriceOracle', [priceOracle._address]);
    ...
    if (kind == 'unitroller') {
        ...
        const maxAssets = etherUnsigned(dfn(opts.maxAssets, 10));
        const liquidationIncentive = etherMantissa(1);
        const comp = opts.comp || await deploy('RewardToken');
        ...
        await send(comptroller, '_setMaxAssets', [maxAssets]);
        await send(comptroller, '_setPriceOracle', [priceOracle._address]);
        await send(comptroller, 'setCompAddress', [comp._address]); // harness only
        await send(comptroller, '_setCompRate', [compRate]);
        // await send(unitroller, '_setComBorrowRatio', [compRate.multipliedBy(0.4)]);
        // await send(unitroller, '_setComSupplyRatio', [compRate.multipliedBy(0.6)]);

        return Object.assign(comptroller, {priceOracle, comp});
    }
}
...
async function makeCToken(opts = {}) {
    ...
    const word = opts.word || "CEtherHarness";
